"""Example of an Optimization Script.

File contains an example of how to use the PowerSystemOptimization class to optimize
the parameters of a power system stabilizer model with respect to system stability.
"""
...
def main(parallel_sims=10):
    """Führt eine Parameteroptimierung der PSS-Parameter für das K2A-Modell durch."""
    ...
    sim.busses[2].models[0].stabilizer.lead_lag1.t_2 = torch.tensor(
        np.random.uniform(
            0.5 * params_original[16], 2.0 * params_original[16], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[2].models[0].stabilizer.lead_lag2.t_1 = torch.tensor(
        np.random.uniform(
            0.5 * params_original[15], 2.0 * params_original[15], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[2].models[0].stabilizer.lead_lag2.t_2 = torch.tensor(
        np.random.uniform(
            0.5 * params_original[17], 2.0 * params_original[17], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    ...
    sim.busses[3].models[0].stabilizer.washout.k_w = torch.tensor(
        np.random.uniform(
            0.5 * params_original[18], 2.0 * params_original[18], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[3].models[0].stabilizer.washout.t_w = torch.tensor(
        np.random.uniform(
            0.5 * params_original[19], 2.0 * params_original[19], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[3].models[0].stabilizer.lead_lag1.t_1 = torch.tensor(
        np.random.uniform(
            0.5 * params_original[20], 2.0 * params_original[20], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[3].models[0].stabilizer.lead_lag1.t_2 = torch.tensor(
        np.random.uniform(
            0.5 * params_original[22], 2.0 * params_original[22], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[3].models[0].stabilizer.lead_lag2.t_1 = torch.tensor(
        np.random.uniform(
            0.5 * params_original[21], 2.0 * params_original[21], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[3].models[0].stabilizer.lead_lag2.t_2 = torch.tensor(
        np.random.uniform(
            0.5 * params_original[23], 2.0 * params_original[23], (parallel_sims, 1)
        ),
        requires_grad=True,
        dtype=torch.complex128,
    )
    ...
    target_tensor = torch.zeros(
        (parallel_sims, int(10 / 0.005), len(record_desired_parameters(sim)))
    )
    ...
    def loss_function(sim_result, target_data):
        ...
        return torch.mean(
            torch.sum(
                torch.abs(
                    target_data[:, int(1.1 / 0.005) :, :]
                    - sim_result[:, int(1.1 / 0.005) :, :]
                ),
                dim=2,
            ),
            axis=1,
