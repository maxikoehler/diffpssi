...
def main(parallel_sims=100):
    """Führt eine Parameteridentifikation für das IBB-Modell durch."""
    ...
    sim.busses[1].models[0].h = torch.tensor(
        np.random.uniform(0.5 * 3.5, 2.0 * 3.5, (parallel_sims, 1)),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[1].models[0].x_d = torch.tensor(
        np.random.uniform(0.5 * 1.81, 2.0 * 1.81, (parallel_sims, 1)),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[1].models[0].x_q = torch.tensor(
        np.random.uniform(0.5 * 1.76, 2.0 * 1.76, (parallel_sims, 1)),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[1].models[0].x_d_t = torch.tensor(
        np.random.uniform(0.5 * 0.3, 2.0 * 0.3, (parallel_sims, 1)),
        requires_grad=True,
        dtype=torch.complex128,
    )
    sim.busses[1].models[0].x_q_t = torch.tensor(
        np.random.uniform(0.5 * 0.65, 2.0 * 0.65, (parallel_sims, 1)),
        requires_grad=True,
        dtype=torch.complex128,
    )
