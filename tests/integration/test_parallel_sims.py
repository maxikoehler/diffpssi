...
class TestSimulationOutput(unittest.TestCase):
    ...
    def test_ieee_9bus_parallel_sim(self):
        ...
        self.assertTrue(
            np.allclose(
                recorder[0, :, :].real, expected_output, atol=self.atol, rtol=self.rtol
            )
        )
        self.assertTrue(
            np.allclose(
                recorder[1, :, :].real, expected_output, atol=self.atol, rtol=self.rtol
            )
        )
        self.assertTrue(
            np.allclose(
                recorder[2, :, :].real, expected_output, atol=self.atol, rtol=self.rtol
            )
        )
