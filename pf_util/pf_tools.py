...
def reset_project(project_path):
    """Reset and re-import the PowerFactory project.

    Deletes the current PowerFactory project and re-imports it to avoid memory leaks.

    Args:
        project_path (str): Path to the PowerFactory project file.

    Returns:
        tuple: (active_project, active_sc, param_ident, grid)
    """
    ...
    if active_project is None:
        raise ImportError(
            "Project could not be loaded. Please check the path to the project."
        )
