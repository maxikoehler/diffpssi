"""Implements the Newton-Raphson load flow algorithm for initializing the power system simulation."""
...
def do_load_flow(ps_sim):
    """
    Perform a load flow calculation for the given power system simulation.

    The load flow is used to initialize the simulation.
    Note: The vectorized version of the load flow is not as easy as the version with "for" loops.

    Args:
        ps_sim: The power system simulation to initialize.

    Returns:
        torch.Tensor: The calculated complex power at each bus.
    """
    ...
    for ia in count(0):
        ...
        # EXPERIMENTAL: Consider Voltage dependent loads in the load flow
        # Updating the v_init of each bus/load model

        # Re-Calculate s_soll with the new initial voltages
        # s_soll = torch.stack([bus.get_lf_power(v_bus[0, i]) for i, bus in enumerate(ps_sim.busses)], axis=1)
        ...
        J = torch.zeros(
            (ps_sim.parallel_sims, dims_jacobian, dims_jacobian), dtype=torch.float64
        )
        ...
        angle_diff = torch.unsqueeze(v_bus_angle_pv_pq, 2) - torch.unsqueeze(
            v_bus_angle_pv_pq, 1
        )
        ...
        J[:, : len(pv_and_pq_bus_ids), : len(pv_and_pq_bus_ids)] = (
            ...
            * v_bus_abs_k
            * (
                y_bus_real_pv_pq * torch.sin(angle_diff)
                - y_bus_imag_pv_pq * torch.cos(angle_diff)
            )
        ).squeeze(-1)
        ...
        angle_diff_pv_pq = torch.unsqueeze(v_bus_angle_pv_pq, 2) - torch.unsqueeze(
            v_bus_angle[:, pq_bus_ids], 1
        )
        ...
        J[
            :,
            diag_indices_pq + len(pv_bus_ids),
            diag_indices_pq + len(pv_and_pq_bus_ids),
        ] = (
        ...
        angle_diff_pq_pv_pq = torch.unsqueeze(
            v_bus_angle[:, pq_bus_ids], 2
        ) - torch.unsqueeze(v_bus_angle_pv_pq, 1)
        ...
        J[:, len(pv_and_pq_bus_ids) :, : len(pv_and_pq_bus_ids)] = (
            ...
            * torch.unsqueeze(v_bus_abs_pv_pq, 1)
            * (
                y_bus_real_pq_pv_pq * torch.cos(angle_diff_pq_pv_pq)
                + y_bus_imag_pq_pv_pq * torch.sin(angle_diff_pq_pv_pq)
            )
        ).squeeze(-1)
        ...
        J[
            :,
            len(pv_and_pq_bus_ids) + diag_indices_pq,
            len(pv_bus_ids) + diag_indices_pq,
        ] = (
        ...
        angle_diff_pq_pq = torch.unsqueeze(
            v_bus_angle[:, pq_bus_ids], 2
        ) - torch.unsqueeze(v_bus_angle[:, pq_bus_ids], 1)
        ...
        J[:, len(pv_and_pq_bus_ids) :, len(pv_and_pq_bus_ids) :] = (
            -torch.unsqueeze(v_bus_abs_pq, 2)
            * (
                y_bus_real_pq_pq * torch.sin(angle_diff_pq_pq)
                - y_bus_imag_pq_pq * torch.cos(angle_diff_pq_pq)
            )
        ).squeeze(-1)
        ...
        error = s_calc - s_soll
        error = torch.concatenate(
            (
                error.real[:, pv_bus_ids],
                error.real[:, pq_bus_ids],
                error.imag[:, pq_bus_ids],
            ),
            axis=1,
        )
        ...
        for i, bus_idx in enumerate(pv_bus_ids):
            ps_sim.busses[bus_idx].update_voltages(
                abs(ps_sim.busses[bus_idx].voltage)
                * torch.exp(
        ...
        for i, bus_idx in enumerate(pq_bus_ids):
            ps_sim.busses[bus_idx].update_voltages(
                ...
                * torch.exp(
                    1j
                    * (
                        torch.angle(ps_sim.busses[bus_idx].voltage)
                        - dx[:, i + len(pv_bus_ids)]
                    )
                )
            )
            ps_sim.busses[bus_idx].update_voltages(
                (
                    abs(ps_sim.busses[bus_idx].voltage)
                    - dx[:, i + len(pv_and_pq_bus_ids)]
                )
                * torch.exp(1j * torch.angle(ps_sim.busses[bus_idx].voltage))
            )

        init_voltages = torch.stack([bus.voltage for bus in ps_sim.busses], axis=1)
        ...
        if torch.max(torch.abs(error)) < 1e-8:
            ...
            if ps_sim.verbose:
                print("=" * 50)
                print(
                    "Load flow finished in {} iterations with error {:.2e} in {:.4f} seconds".format(
                        ia, torch.max(torch.abs(error)), time.time() - lf_time
                    )
                )
                print("=" * 50)
            break
