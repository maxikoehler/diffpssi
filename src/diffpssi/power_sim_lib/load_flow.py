...
def do_load_flow(ps_sim):
    ...
    The load flow is used to initialize the simulation. Note: The vectorized version of the load flow is not as easy as the version with "for" loops.
    ...
    for ia in count(0):
        ...
        if torch.max(torch.abs(error)) < 1e-8:
            ...
            if ps_sim.verbose:
                print("=" * 50)
                print(
                    "Load flow finished in {} iterations with error {} in {} seconds".format(
