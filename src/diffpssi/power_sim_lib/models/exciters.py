"""Contain the SEXS (Simplified Excitation System) model for power system simulations.
...
class SEXS(object):
    """
    Represent the SEXS (Simplified Excitation System) model in power system simulations.
    ...
    def __init__(
        ...
        """
        Initialize the SEXS model with specified parameters.
        ...
        self.pt1_lim = PT1Limited(
            t_pt1=self.t_e, gain_pt1=self.k, lim_min=self.e_min, lim_max=self.e_max
        )
    ...
    def differential(self):
        """
        Compute the differential equations for the SEXS model.

        Returns:
            torch.Tensor: A tensor containing the derivatives of the state variables.
        """
        return torch.concatenate(
            [self.lead_lag.differential(), self.pt1_lim.differential()], axis=1
        )
    ...
    def get_state_vector(self):
        ...
        """
        return torch.concatenate(
            [self.lead_lag.get_state_vector(), self.pt1_lim.get_state_vector()], axis=1
        )
    ...
    def get_output(self, input_value):
        """
        Compute the output of the SEXS model given an input value.
    ...
    def enable_parallel_simulation(self, parallel_sims):
        ...
        """
        self.v_setpoint = (
            torch.ones((parallel_sims, 1), dtype=torch.float64) * self.v_setpoint
        )
    ...
    def initialize(self, e_fd):
        """
        Initialize the SEXS model for simulation.
