...
class SynchMachine:
    ...
    The SynchMachine class models a synchronous generator or motor,
    including its dynamic behavior and interactions with external control systems
    such as exciters, governors, and power system stabilizers.

    Attributes:
        s_n, v_n, p_soll_mw, v_soll, h, d, x_d, x_q, x_d_t, x_q_t, x_d_st, x_q_st, t_d0_t, t_q0_t,
        t_d0_st, t_q0_st:
    ...
    def __init__(
        ...
        """
        Initialize the SynchMachine object with specified parameters.
        ...
        Args:
            ...
            x_q_st (float, optional): The quadrature-axis subtransient reactance
            of the machine in pu.
            t_d0_t (float, optional): The direct-axis open-circuit time constant
            of the machine in seconds.
            t_q0_t (float, optional): The quadrature-axis open-circuit time constant
            of the machine in seconds.
            t_d0_st (float, optional): The direct-axis open-circuit subtransient time constant
            of the machine in
            seconds.
            t_q0_st (float, optional): The quadrature-axis open-circuit subtransient time constant
            of the machine in seconds.
        """
    ...
    def differential(self):
        """
        Compute the differential equations governing the dynamics of the synchronous machine.

        This method calculates the time derivatives of the state variables
        of the synchronous machine,
        including its interaction with the exciter, governor, and power system stabilizer models
        if they are present.
        ...
        d_e_q_t = (
            1
            / self.t_d0_t
            * (self.e_fd - self.e_q_t - self.i_d * (self.x_d - self.x_d_t))
        )
        d_e_d_t = 1 / self.t_q0_t * (-self.e_d_t + self.i_q * (self.x_q - self.x_q_t))
        d_e_q_st = (
            1
            / self.t_d0_st
            * (self.e_q_t - self.e_q_st - self.i_d * (self.x_d_t - self.x_d_st))
        )
        d_e_d_st = (
            1
            / self.t_q0_st
            * (self.e_d_t - self.e_d_st + self.i_q * (self.x_q_t - self.x_q_st))
        )

        deriv_vec = torch.stack(
            [d_omega, d_delta, d_e_q_t, d_e_d_t, d_e_q_st, d_e_d_st], axis=1
        )
        if self.exciter:
            deriv_vec = torch.concatenate(
                (deriv_vec, self.exciter.differential()), axis=1
            )
        if self.governor:
            deriv_vec = torch.concatenate(
                (deriv_vec, self.governor.differential()), axis=1
            )
        if self.stabilizer:
            deriv_vec = torch.concatenate(
                (deriv_vec, self.stabilizer.differential()), axis=1
            )
    ...
    def add_exciter(self, exciter_model):
        """
        Associate an exciter model with the synchronous machine.
    ...
    def add_governor(self, governor_model):
        """
        Associate a governor model with the synchronous machine.
    ...
    def add_pss(self, pss_model):
        """
        Associate a power system stabilizer model with the synchronous machine.
    ...
    def set_state_vector(self, x):
        """
        Set the state vector of the synchronous machine based on provided values.
    ...
    def get_state_vector(self):
        """
        Retrieve the current state vector of the synchronous machine.

        Returns:
            torch.Tensor: The current state vector of the machine.
        """
        state_vec = torch.stack(
            [self.omega, self.delta, self.e_q_t, self.e_d_t, self.e_q_st, self.e_d_st],
            axis=1,
        )
        if self.exciter:
            state_vec = torch.concatenate(
                (state_vec, self.exciter.get_state_vector()), axis=1
            )
        if self.governor:
            state_vec = torch.concatenate(
                (state_vec, self.governor.get_state_vector()), axis=1
            )
        if self.stabilizer:
            state_vec = torch.concatenate(
                (state_vec, self.stabilizer.get_state_vector()), axis=1
            )
    ...
    def calc_current_injections(self):
        """
        Calculate the current injections from the synchronous machine into the network.
        ...
        i_q = (
            self.e_d_st
            / (1j * self.x_q_st)
            * torch.exp(1j * (self.delta - torch.pi / 2))
        )
    ...
    def update_internal_vars(self, v_bb):
        """
        Update internal variables of the synchronous machine based on the voltage at the busbar.
        ...
        e_st = self.e_q_st * torch.exp(1j * self.delta) + self.e_d_st * torch.exp(
            1j * (self.delta - torch.pi / 2)
        )
    ...
    def initialize(self, s_calc, v_bb):
        """
        Initialize the synchronous machine's states based on provided conditions.
    ...
    def get_admittance(self, dyn):
        """
        Return the admittance value of the synchronous machine.

        Args:
            dyn (bool): Flag indicating whether to calculate dynamic (True)
            or static (False) admittance.
    ...
    def get_lf_power(self):
        """
        Retrieve the load flow power of the synchronous machine.
    ...
    def enable_parallel_simulation(self, parallel_sims):
        """
        Enable running parallel simulations for the synchronous machine.
        ...
        self.p_soll_mw = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.p_soll_mw
        )
        self.v_soll = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.v_soll
        )
        ...
        self.x_d_st = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.x_d_st
        )
        self.x_q_st = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.x_q_st
        )
        self.t_d0_t = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.t_d0_t
        )
        self.t_q0_t = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.t_q0_t
        )
        self.t_d0_st = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.t_d0_st
        )
        self.t_q0_st = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.t_q0_st
        )
        ...
        self.e_q_st = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.e_q_st
        )
        self.e_d_st = (
            torch.ones((parallel_sims, 1), dtype=torch.complex128) * self.e_d_st
        )
